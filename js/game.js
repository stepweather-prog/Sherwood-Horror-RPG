...
// Коридор 5x5 (от 1 до 5) — кольцо вокруг дуба
for (let y = 1; y <= 5; y++) {
    for (let x = 1; x <= 5; x++) {
        map[y][x] = 0;
    }
...
// Дуб занимает центр (3,3) — непроходим, но НЕ стена
const oak = { x: 3.5, y: 3.5 };

// Иконки на внешних стенах
const buildings = [
    { x: 1, y: 0, icon: 'Квесты', name: 'Квесты' },
    { x: 3, y: 0, icon: 'Арена', name: 'Арена' },
    { x: 5, y: 0, icon: 'Рынок', name: 'Рынок' },
    { x: 6, y: 1, icon: 'Таверна', name: 'Таверна' },
    { x: 6, y: 3, icon: 'Кузница', name: 'Кузница' },
    { x: 6, y: 5, icon: 'Тренировка', name: 'Тренировка' },
    { x: 5, y: 6, icon: 'Бестиарий', name: 'Бестиарий' },
    { x: 3, y: 6, icon: 'Очаг', name: 'Очаг' },
    { x: 1, y: 6, icon: 'Порталы', name: 'Порталы' },
    { x: 0, y: 5, icon: 'Чат', name: 'Чат' },
    { x: 0, y: 3, icon: 'Профиль', name: 'Профиль' },
    { x: 0, y: 1, icon: 'Рейд', name: 'Рейд' },
    { x: 2, y: 0, icon: 'Подземка', name: 'Подземка' },
    { x: 4, y: 0, icon: 'Сумка', name: 'Сумка' },
    { x: 6, y: 2, icon: 'Настройки', name: 'Настройки' },
    { x: 6, y: 4, icon: 'Таланты', name: 'Таланты' },
    { x: 4, y: 6, icon: 'Бестиарий', name: 'Бестиарий' },
    { x: 2, y: 6, icon: 'Ежедневные', name: 'Ежедневные' },
];
...
function render() {
    ...
    for (let x = 0; x < W; x++) {
        ...
        const lineHeight = Math.floor(H / correctedDistance);
...
function canMoveTo(x, y) {
    ...
    // Запрет ходить через дуб
    if (mx === 3 && my === 3) return false;
...
document.addEventListener('keydown', (e) => {
    ...
    switch(e.key.toLowerCase()) {
        case 'w': case 'С†': case 'arrowup':
            e.preventDefault();
            stepForward();
            break;
        case 'a': case 'С„': case 'arrowleft':
            e.preventDefault();
            turnLeft();
            break;
        case 'd': case 'РІ': case 'arrowright':
            e.preventDefault();
            turnRight();
            break;
        case 's': case 'С‹': case 'arrowdown':
            e.preventDefault();
