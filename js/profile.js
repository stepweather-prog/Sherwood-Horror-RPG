...
Sherwood.ProfileUI = {
    show: function() {
        ...
        var h = '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:4px;margin-bottom:8px;">';
    ...
    _showSkinSelector: function() {
        ...
        for (var i = 0; i < unlockedSkins.length; i++) {
            ...
            h += '<img src="assets/hero_skins/' + sid + '.png" style="width:64px;height:64px;object-fit:contain;border-radius:4px;" onerror="this.src=\'assets/hero_skins/skin1_01.png\'"><div style="color:' + (isActive ? '#ffd700' : '#e0c080') + ';font-size:0.75em;font-weight:bold;margin-top:4px;">' + skinName + '</div>';
            if (isActive) { h += '<div style="color:#ffd700;font-size:0.6em;margin-top:2px;">Установлен</div>'; }
            else { h += '<div style="color:#4caf50;font-size:0.6em;margin-top:2px;">Нажми чтобы надеть</div>'; }
            h += '</div>';
        }
        h += '</div>';
    ...
    _showAllTrophies: function() {
        ...
        if (trophies.length === 0) { h += '<div style="text-align:center;padding:20px;"><img src="assets/all_trophies/asset_isolated_on_a_solid.png" style="width:180px;height:180px;object-fit:contain;"><div style="color:#aaa;margin-top:8px;">Нет трофеев</div></div>'; }
    ...
    _showTalents: function() {
        ...
        for (var id in skills) {
            ...
            h += '<img src="' + s.icon + '" style="width:44px;height:44px;object-fit:contain;"><div style="flex:1;"><div style="color:#e0c080;">' + s.name + '</div><div style="color:#aaa;font-size:0.7em;">' + s.description + '</div></div>';
    ...
    _toggleTalent: function(id) {
        var player = Sherwood.getPlayer();
        if (!player.activeSkills) player.activeSkills = {};
        if (player.activeSkills[id] === false) { player.activeSkills[id] = true; }
        else { player.activeSkills[id] = false; }
        Sherwood.saveGame();
        this._showTalents();
    },

    _showWallet: function() {
        if (typeof Sherwood.WalletUI !== 'undefined') { Sherwood.WalletUI.show(); }
        else { SherwoodUI._showPlaceholder('Кесет', 'wallet', 'Sherwood.ProfileUI.show()'); }
    },

    _showBestiary: function() {
        if (typeof Sherwood.BestiaryUI !== 'undefined') { Sherwood.BestiaryUI.show(); }
        else { SherwoodUI._showPlaceholder('Бестиарий', 'bestiary', 'Sherwood.ProfileUI.show()'); }
    }
};
