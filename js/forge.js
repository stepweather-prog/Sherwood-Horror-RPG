...
Sherwood.ForgeUI = {
    show: function() {
        ...
        if (ring) { h += '<button onclick="Sherwood.ForgeUI._enhanceEquipped(\'ring\')" style="background:#c9a040;border:none;border-radius:8px;padding:12px 20px;color:#000;font-weight:bold;cursor:pointer;font-size:0.85em;width:80%;max-width:300px;">Заточить кольцо (+' + ringLevel + ') — ' + ringCost + ' сер.</button>'; }
        else { h += '<div style="background:rgba(0,0,0,0.5);border:1px solid #555;border-radius:8px;padding:12px 20px;color:#888;font-size:0.85em;width:80%;max-width:300px;text-align:center;">Нет кольца</div>'; }
        if (amulet) { h += '<button onclick="Sherwood.ForgeUI._enhanceEquipped(\'amulet\')" style="background:#c9a040;border:none;border-radius:8px;padding:12px 20px;color:#000;font-weight:bold;cursor:pointer;font-size:0.85em;width:80%;max-width:300px;">Заточить амулет (+' + amuletLevel + ') — ' + amuletCost + ' сер.</button>'; }
        else { h += '<div style="background:rgba(0,0,0,0.5);border:1px solid #555;border-radius:8px;padding:12px 20px;color:#888;font-size:0.85em;width:80%;max-width:300px;text-align:center;">Нет амулета</div>'; }
    ...
    _showSkinCrafting: function() {
        ...
        for (var i = 0; i < skins.length; i++) {
            ...
            if (owned) { h += '<div style="color:#4caf50;font-size:0.7em;font-weight:bold;margin-top:4px;">ВЫКОВАН</div>'; }
            else { h += '<div style="color:#aaa;font-size:0.65em;margin-top:4px;">Чертежей: ' + skin.cost.drawings + ' | Скрижалей: ' + skin.cost.tablets + '</div><button onclick="Sherwood.ForgeUI._craftSkin(\'' + skin.id + '\')" style="margin-top:4px;background:#c9a040;border:none;border-radius:4px;padding:6px 16px;color:#000;cursor:pointer;font-size:0.7em;font-weight:bold;">Создать</button>'; }
            h += '</div>';
        }
        h += '</div>';
    ...
    _showRingCrafting: function() {
        ...
        for (var i = 0; i < rings.length; i++) {
            ...
            h += '<div style="background:rgba(0,0,0,0.5);border:2px solid ' + (owned ? '#4caf50' : '#555') + ';border-radius:8px;padding:12px;width:80%;text-align:center;">';
            h += '<img src="' + ring.icon + '" style="width:64px;height:64px;object-fit:contain;border-radius:4px;"><div style="color:#e0c080;font-size:0.8em;font-weight:bold;margin-top:4px;">' + ring.name + '</div><div style="color:#aaa;font-size:0.65em;margin-top:2px;">АТК +' + ring.stats.attack + ' | ЗЩТ +' + ring.stats.defense + '</div>';
            if (owned) { h += '<div style="color:#4caf50;font-size:0.7em;font-weight:bold;margin-top:4px;">ВЫКОВАН</div>'; }
            else { h += '<div style="color:#aaa;font-size:0.65em;margin-top:4px;">Скрижалей: 10</div><button onclick="Sherwood.ForgeUI._buyRing(\'' + ring.id + '\')" style="margin-top:4px;background:#c9a040;border:none;border-radius:4px;padding:6px 16px;color:#000;cursor:pointer;font-size:0.7em;font-weight:bold;">Создать</button>'; }
            h += '</div>';
        }
        h += '</div>';
    ...
    _showAmuletCrafting: function() {
        ...
        for (var i = 0; i < amulets.length; i++) {
            ...
            h += '<div style="background:rgba(0,0,0,0.5);border:2px solid ' + (owned ? '#4caf50' : '#555') + ';border-radius:8px;padding:12px;width:80%;text-align:center;">';
            h += '<img src="' + amulet.icon + '" style="width:64px;height:64px;object-fit:contain;border-radius:4px;"><div style="color:#e0c080;font-size:0.8em;font-weight:bold;margin-top:4px;">' + amulet.name + '</div><div style="color:#aaa;font-size:0.65em;margin-top:2px;">HP +' + amulet.stats.hp + ' | ЗЩТ +' + amulet.stats.defense + '</div>';
            if (owned) { h += '<div style="color:#4caf50;font-size:0.7em;font-weight:bold;margin-top:4px;">ВЫКОВАН</div>'; }
            else { h += '<div style="color:#aaa;font-size:0.65em;margin-top:4px;">Скрижалей: 10</div><button onclick="Sherwood.ForgeUI._buyAmulet(\'' + amulet.id + '\')" style="margin-top:4px;background:#c9a040;border:none;border-radius:4px;padding:6px 16px;color:#000;cursor:pointer;font-size:0.7em;font-weight:bold;">Создать</button>'; }
            h += '</div>';
        }
        h += '</div>';
    ...
    _craftArrow: function() {
        ...
        if (arrowInfo.canCraft > 0) {
            ...
            if (r.success) { if (info) info.textContent = 'Создано стрел: ' + (r.crafted || 1); }
